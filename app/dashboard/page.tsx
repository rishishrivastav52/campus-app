  ...
  return (
    <div className="grid md:grid-cols-2 gap-6">
      ...
      <div className="card h-fit">
        ...
        <form action={requestResource} className="flex items-center gap-2 mb-4">
          <select name="resourceId" className="input" required defaultValue="">
          ...
          {myRequests.map((req) => (
            <div key={req.id} className="border rounded-lg px-3 py-2">
              <div className="flex items-center justify-between gap-2">
                <span className="text-sm font-medium">{req.resource.name} × {req.quantity}</span>
                <ReqBadge status={req.status} />
              </div>
              {req.status === "APPROVED" && (
                <>
                  <p className="text-xs text-gray-500 mt-1">📍 Pick up at: {req.resource.location}</p>
        ...
        <div className="space-y-3">
          ...
          {issues.map((issue) => (
            <div key={issue.id} className="card">
              ...
              <form action={assignIssue} className="flex items-center gap-2 mt-3 border-t pt-3">
            ...
            <tbody>
              ...
              {resourceRequests.map((req) => (
                <tr key={req.id} className="border-t">
                  ...
                  <td className="px-4 py-2">{req.resource.name}</td>
